 ...
 */
  private ArrayList<InventoryItem> roomInventory;
 ...
 */
    ...
    roomInventory = new ArrayList<InventoryItem>();
  }
 ...
 * Hint: use the custom constructor above with the "this" key word
 *
 */
 ...
 */
  ...
  public InventoryItem removeObject(String objName) {
    for(int i = 0; i < roomInventory.size(); i++) {
      if (roomInventory.get(i).getName().equals(objName)) {
 ...
 */
  ...
  public String getNamesOfObjectsInRoom() {
    String inventory = "Items in room are : ";
    for (int i = 0; i < roomInventory.size(); i++) {
      inventory += roomInventory.get(i).getName() + ", ";
    }
    return inventory;
  }
}